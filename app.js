...
function showView(name) {
  ...
  const titles = {dashboard:'Dashboard',records:'Records',portal:'Record',users:'Users',settings:'Settings'};
  const el = document.getElementById('mobile-page-title');
  if (el) el.textContent = titles[name] || 'GeniusOne Portal';
...
function renderTable(container, records, compact = false) {
    ...
    <tr>
      ...
      <td>
        <div class="actions">
          ...
          ${currentUser?.role === 'admin' ? `<button class="btn btn-sm btn-danger" onclick="deleteRecord('${r.id}')">Del</button>` : ''}
        </div>
      </td>
    </tr>`).join('');
  ...
  container.innerHTML = `<div class="table-wrap">${tableHTML}</div>`;
}
  ...
  try {
    ...
    document.getElementById('users-table').innerHTML = `
      <div class="table-wrap"><table class="data-table"><thead><tr>
        <th>Name</th><th>Email</th><th>Role</th><th>Status</th><th>Created</th><th style="min-width:220px"></th>
...
function showAddUserModal() {
  ...
  document.getElementById('modal-body').innerHTML = `
    <div class="field"><label>Full Name</label><input type="text" id="mu-name" placeholder="Full name"/></div>
    <div class="field"><label>Email</label><input type="email" id="mu-email" placeholder="user@email.com"/></div>
    <div class="field"><label>Password</label><input type="password" id="mu-password" placeholder="Password"/></div>
    <div class="field"><label>Role</label><select id="mu-role"><option value="typist">Typist</option><option value="admin">Admin</option></select></div>
    <div id="mu-error" class="alert alert-error hidden"></div>
    <div class="modal-actions">
      <button class="btn btn-secondary" onclick="closeModal()">Cancel</button>
